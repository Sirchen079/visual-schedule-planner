...
**当前源码版本：2.15.0 · 桌面构建目标：Windows x64 · MIT 开源**

2.15.0 增加 GitHub 自动更新、可恢复的 AI 提问卡片和标准 Markdown 渲染，并通过按需加载工具、完整结果分页与提前压缩降低长对话的上下文占用。
...
从 [GitHub Releases](https://github.com/Sirchen079/visual-schedule-planner/releases) 下载 `zhishi-Setup-2.15.0.exe`，双击安装，再从桌面快捷方式打开“知时”。安装版自带运行环境，不需要另外安装 Python 或 Node.js。
...
安装包输出到 `electron-v2/dist/zhishi-Setup-2.15.0.exe`。构建产物、数据库和密钥不应提交到仓库。
