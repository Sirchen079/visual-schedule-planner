...
- 本地存储：所有数据保存在本机 `%APPDATA%\知时\data\`，卸载程序不会删除。
...
双击 `知时 Setup.exe`，按向导选择目录完成安装。安装程序在复制文件前会自动关闭正在运行的旧版知时（先优雅退出并备份，再覆盖），无需手动结束进程。
...
冷备份或迁移：复制整个 `%APPDATA%\知时\` 目录到新机器的 `%APPDATA%\知时\` 即可。

### 升级与卸载

- 升级：直接运行新版 `知时 Setup.exe`，安装程序自动关闭旧版（含备份）并覆盖；用户数据保留在 `%APPDATA%\知时\`，新版自动读取，无需迁移。
- 卸载：从「设置 → 应用」卸载，仅删除程序文件，**用户数据保留**；如需彻底清除，手动删除 `%APPDATA%\知时\`。
...
├── data/           # 开发模式运行数据（gitignore；桌面应用用 %APPDATA%）
└── docs/           # 项目文档
```
