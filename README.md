...
**当前源码版本：2.16.0 · 桌面构建目标：Windows x64 · MIT 开源**

2.16.0 完善 AI 工具流程：常见任务提供推荐调用顺序，工具结果直接给出下一步参数；任务、日程与打卡重试自动去重，批量排期在保存前重新校验，失败时整批回滚。单个 MCP 服务故障时，其余工具仍可继续工作。
...
从 [GitHub Releases](https://github.com/Sirchen079/visual-schedule-planner/releases) 下载 `zhishi-Setup-2.16.0.exe`，双击安装，再从桌面快捷方式打开“知时”。安装版自带运行环境，不需要另外安装 Python 或 Node.js。
