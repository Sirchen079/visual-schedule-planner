...
- 模型原生联网：可开启 OpenAI / Claude / Kimi 等兼容接口自带的联网搜索能力，并可用 JSON 补充接口私有参数
- 对话式管理：通过自然语言查看、创建、规划任务，整理资料，生成提醒与时间安排
- 任务拆解：AI 可创建真实子任务，工具执行失败时会自动把错误反馈给模型重试修正
- 会话历史：可开始新聊天，也可回溯最近 50 次 AI 会话
...
- 在模型配置中开启 **模型原生联网搜索** 后，后端会把联网参数随聊天请求一并发送给模型接口。
- 默认会按当前 Provider 加入常见原生搜索参数：OpenAI Chat 使用 `web_search_options`，OpenAI Responses 使用内置 `web_search_preview` 工具，Claude Messages 使用 `web_search_20250305` 工具。
- 如 Kimi Code 或其他兼容接口需要自己的字段，可在 **原生联网参数 JSON** 中覆盖或补充请求体字段。
- 联网搜索由模型服务商自身执行，本项目不额外接入第三方搜索服务。
