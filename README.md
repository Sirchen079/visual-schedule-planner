...
**当前源码版本：2.17.0 · 桌面构建目标：Windows x64 · MIT 开源**

2.17.0 将使用教程与 AI 接入说明内置到应用，断网也能阅读。新安装且没有历史数据时自动显示新手指引，带你认识界面、保存第一条待办、了解 AI 接入；完成或跳过后不再自动出现，也能手动重看。

[知时使用教程](docs/getting-started.md) · [AI 接入新手指南](docs/ai-api-guide.md) · [源码构建](#从源码运行) · [问题反馈](https://github.com/Sirchen079/visual-schedule-planner/issues)
...
从 [GitHub Releases](https://github.com/Sirchen079/visual-schedule-planner/releases) 下载 `zhishi-Setup-2.17.0.exe`，双击安装，再从桌面快捷方式打开“知时”。安装版自带运行环境，不需要另外安装 Python 或 Node.js。

首次从 2.14.x 升级需手动安装此版本。2.15.0 起，安装版在启动后和运行期间每六小时检查 GitHub 新版；也可在 **设置 → 悬浮窗与桌面通知 → 检查更新** 手动检查。点击“下载并安装”后会校验安装包、保存主窗口与悬浮窗草稿，再打开安装程序。任务仍在运行或草稿未保存时会保留下载，等你点击“安装并重启”。网络不可用时可重试或从发布页手动下载。

GitHub 的 “Source code” 压缩包是源码，不能直接当作安装程序运行。

2.x 使用独立数据目录，**不兼容、不会自动导入或迁移 1.x 数据**。如仍在使用旧版，请保留原应用和原数据备份。

### 2. 先试一个简单操作

第一次安装、没有历史数据时，会自动打开新手指引。也可以随时点顶部 **使用教程** 查看内置教程，或在弹窗中点 **新手指引** 重新学习。设置页同样提供这两个按钮，悬浮窗的“使用教程”会打开主窗口教程。
...
| AI 为什么会显示问题卡片？ | 需要补充信息时可单选、多选或自由填写；提交后继续原任务，也可明确跳过。提问与操作审批分别处理，重启后仍可继续回答。 |
| AI 操作失败会不会重复创建？ | 同一请求内，相同参数的任务、日程、打卡和支持的批量操作会复用回执；新消息中的新操作单独处理。外部服务写入结果不明时先核对状态，不自动重复提交。 |
| 怎样查看执行计划？ | 对话中出现“执行计划”后可展开查看。“有执行记录”表示关联了本会话成功的工具调用，“AI 汇报”表示尚未关联工具回执。 |
| 长对话如何处理？ | 工具按需加载，大结果保存完整原文并分页读取；已知模型窗口接近预算时会显示“整理上下文”。压缩后保留当前输入和未完成调用，可读取原始记录核对。本地 token 数是估算，模型窗口仍以服务商配置为准。 |
...
安装包输出到 `electron-v2/dist/zhishi-Setup-2.16.0.exe`。构建产物、数据库和密钥不应提交到仓库。
