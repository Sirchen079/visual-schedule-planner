...
- 重复任务：每天 / 每个工作日 / 每周 / 每月（可设间隔），完成时自动生成下一期实例。
- 提醒：支持纯日期或「截止时刻」，并可设置多次提醒（截止时 / 提前 30 分钟 / 提前 2 小时 / 提前 1 天 / 提前 3 天）。
- 自然语言快速创建：输入「明天下午3点写周报 #工作 !高」自动解析日期、时刻、标签与优先级。
- 多视图：总览、日历、时间轴，查看截止日期、进度与并行安排。
- 时间轴：整条拖动平移起止日期、边缘拉伸调整开始/结束、今天标线。
- 日历月视图：拖拽条目直接改期，双击空白格按该日期快速创建。
- 看板列内手动排序：同列拖拽排序自动持久化。
- 标签：按标签组织任务，日历视图按标签颜色显示。
- 看板暖心提醒：看板侧栏按时辰（清晨 / 上午 / 午间 / 午后 / 夜晚 / 深夜）与任务实况（逾期、今日到期、进行中、清空）落一句应景寄语；连续专注过久会轻声劝歇，深夜只劝早睡。连续工作时长基于系统级空闲感知——你在任意窗口认真做事都算数，真的离开电脑才会中断。

### 习惯与日记

- 习惯打卡：每日 / 每周目标次数，打卡累积与撤销，连续纪录（streak）与近 12 周热力格子。
- 日记：一天一篇 Markdown 日记，心情标记，编辑 / 预览切换，切换日期自动保存。
- AI 可代为打卡与记日记：助手可直接调用习惯与日记工具，并在上下文中感知今日打卡与最近日记。

### 目标与专注

- 目标 OKR：定性目标 + 可量化关键结果；KR 支持手动填报、关联标签任务完成数、关联习惯打卡数三种进度模式，自动滚动总进度。
- 番茄钟：任务卡 / 任务详情一键「开始专注」，左下悬浮计时器（25 分钟工作 + 5 分钟休息，支持 +5 分钟），刷新不丢。
- 时间投入：按日时长柱状、标签分布、预估 vs 实际对照（任务可填预估耗时），看清时间去向并校准估算。

### 功能管理

- 功能开关：习惯 / 日记 / 目标 / 番茄钟 / 内嵌 AI 动作默认开启，每日晨报 / 秘书自动档 / 伴随联动需显式开启——均在「功能管理」（顶栏独立入口）按需开关；关闭只隐藏入口（导航、快捷键、AI 工具与上下文），数据完整保留。
- 设置与功能分离：「设置」管偏好（开机自启、关闭行为、报告参数、数据互通），「功能管理」管模块开关。

### 数据分析

- 生产力分析：近 30 天完成 / 新增趋势、近 12 周完成热力图、标签分布、优先级 × 状态统计（总览页）。
- 风险预警：规则化打分（逾期 / 临期低进度 / 停滞 / 高优未安排），总览页与晨报同步呈现。
- Token 用量：助手「用量」页按日统计、按模型汇总，可在模型配置中填写单价（每百万 tokens）估算成本。

### 数据互通

- iCal 导出：全部任务一键导出为 .ics 日历文件（含起止 / 截止日期、标签、优先级）。
- iCal 导入：从 .ics 文件批量创建任务（设置 → 数据互通），单文件上限 50MB（可经 `APP_MAX_ICAL_MB` 调整）。

### 交互效率

- 命令面板：`Ctrl/⌘ + K` 唤起，支持视图跳转、自然语言新建任务、任务搜索与常用动作。
- 全局快速捕获（桌面端）：`Ctrl + Shift + A` 随时唤出小窗，一句话（支持自然语言解析）回车即建任务。
- 快捷键：数字 `1-9` 切换视图，`?` 查看全部快捷键。
- 任务卡右键菜单：状态推进、改期（今天 / 明天 / 下周一 / 清除）、设优先级、删除。

### 通知中心

- 铃铛未读角标：到点提醒自动落库，未读数量实时可见。
- 通知历史：错过的提醒随时回溯，支持单条已读与全部已读。
...
- **双助手可选**：知时助手（原版问答式，有求必应有问必答）与知时代理（贴身秘书，主动关联全域数据、办妥事务），助手面板头部一键切换。
- 悬浮式助手：展开对话、全屏使用、拖动、查看历史会话与新建会话。
- 会话管理：历史会话支持重命名与删除。
- **秘书自动档**（知时代理专属，可选）：每天首次启动，AI 主动办妥今日规划——智能排程进日历、大任务拆成子任务，结果卡片逐项可撤销；开启即授权低风险操作直接执行。
- **内嵌 AI 动作**：任务详情一键「AI 拆解」「AI 排程」直接落库；日记编辑器「AI 本日小结」按今日完成/打卡/计时生成草稿。
- **伴随联动**（知时代理专属，可选）：番茄钟结束时结合任务进度给个性化收束语。
- **幕僚洞察**（知时代理专属）：断签风险、KR 周期落后、预估偏差、计时异常、情绪线索等跨域观察，自动融入对话、晨报与周报。
- 每日晨报（可选）：在「功能管理」中开启后，每天首次启动生成今日简报；未配置 AI 时降级为规则文案，每天最多一次模型调用。
...
- **流式响应**：回复逐字出现、工具调用实时卡片化呈现（状态/参数/结果），长任务不再静默等待。
- **可中断**：处理中可随时点「停止」中断后续步骤（已执行的副作用不回滚）；Esc 只关窗不杀请求。
- **确认后自动续跑**：危险操作确认/拒绝后，被暂缓的安全工具与模型总结自动续跑，无需重发。
- **原生 Function Calling**：工具调用走 OpenAI/Anthropic 原生协议（已下线旧的 JSON-plan 兼容模式）。
...
| iCal 导入上限 | `APP_MAX_ICAL_MB` | `50` | iCal 导入单文件大小上限 |
...
- 工具调用采用**原生 function-calling**（OpenAI Chat / OpenAI Responses / Claude Messages 三套协议适配），工具清单由单一数据源 `tool_registry` 统一维护，MCP 工具以 `mcp__s{id}__原名` 命名空间一并暴露。模型直接返回结构化 tool_calls，不再依赖提示词 JSON-plan。
- 一次请求最多连续 5 轮（规划 → 低风险工具执行 → 结果观察 → 下一步）。
- 已成功的重复工具调用会被跳过。
- 危险操作不执行，改为生成确认卡片；用户可**确认执行**或**拒绝**，确认/拒绝后助手会自动续跑被暂缓的安全工具并给出收尾回复（`POST /ai/chat/resume`）。
- 助手回复按 Markdown 渲染（标题/列表/引用/粗体/行内代码/链接），工具调用以可视化卡片呈现状态与结果。

## MCP 服务器配置

知时助手支持接入 [Model Context Protocol](https://modelcontextprotocol.io)（MCP）服务器，扩展文件系统、网页抓取、数据库查询等第三方工具能力。在助手「设置 → MCP 服务器」中新增：

- **stdio（本地命令）**：填写可执行命令与参数数组，如命令 `npx`、参数 `["-y", "@modelcontextprotocol/server-filesystem", "D:/docs"]`。需本机已安装对应命令（`npx` 需 Node、`uvx` 需 uv）。更多服务器见 [MCP 官方服务器列表](https://github.com/modelcontextprotocol/servers)。
- **http（远程地址）**：填写服务器 URL（仅 `http(s)://`）与可选请求头。
- 环境变量与请求头属于敏感字段，保存后加密存储、界面返回脱敏值，编辑时留空即保持不变（与 API Key 同款交互）。
- 每个服务器可单独启用 / 停用，并选择「只读工具免确认」（仅对带只读标记的工具生效）。

生效方式：启用服务器的工具会以 `mcp__` 前缀注入对话（命名空间隔离，避免与内置工具冲突），工具清单带 60 秒缓存。出于安全考虑，MCP 工具**默认需经确认卡片调用**；服务器开启「只读工具免确认」且工具声明为只读时方可直接执行。安装版中 stdio 服务器以后端子进程方式运行，依赖用户机器上已安装的对应命令。
...
- MCP 工具默认同样需经确认卡片调用；stdio 命令走参数数组，不拼 shell；环境变量与请求头一律脱敏返回。
...
| 前端 | Vue 3、Vite、vuedraggable、ECharts |
