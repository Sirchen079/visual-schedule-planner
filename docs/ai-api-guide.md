...
这份指南按当前知时设置界面编写。你不需要写代码，只需从模型服务商处取得几个配置值，再填入知时。教程已随安装包提供，断网也能打开阅读；实际使用外部 AI 仍需要网络。

[返回首页](../README.md)

## 还没有账号或密钥，怎么办

**AI 是可选功能。** 你可以先关闭本弹窗，手动记待办、查看日历、记账，之后再来接入。

要使用 AI，需要先有一个提供模型接口的服务账号。去你准备使用的服务商网站注册或登录，在它的控制台找到“API Key / 密钥管理”，创建一个密钥，并查看接口接入文档。

平时在网页里和 AI 聊天的账号或订阅，**不一定自带 API 使用权限或额度**。知时不会提供免费密钥，也不会代你购买额度；具体是否收费，以你选择的服务商说明为准。

准备好这些资料再继续：**API Key、Base URL、模型名称、接口格式**。它们应来自同一个服务商。不知道某一项时，先查服务商文档或询问其客服，不要凭感觉填写。

## 复制和粘贴，不用手打长串文字

1. 服务商页面有“复制”按钮时，直接点它。
2. 回到知时，先点击要填写的输入框。
3. 按住键盘 **Ctrl**，再按一下 **V**，即可粘贴；也可以在输入框里右键，选择“粘贴”。
4. 如果填错框，在该输入框内按 **Ctrl + A** 选中全部，再粘贴正确内容。

密钥框显示圆点是正常的。不要为了让别人排错，把密钥截图或发出去。先复制一个值、填一个框，避免四项资料混在一起。
...
1. 点击知时左侧的 **设置**，再点页面上方分类里的 **AI 模型**，最后点 **添加配置**。也可以点本教程底部的“打开 AI 模型设置”。
...
| 400，提示 regex lookaround is not supported | 升级到 2.14.3 或更新版本。旧版金额工具定义含部分服务不支持的正则，可能使普通对话也失败。 |
