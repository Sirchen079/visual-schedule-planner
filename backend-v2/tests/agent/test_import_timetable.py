# tests/agent/test_import_timetable.py
import json
...
ENTRIES = [
    ...
    {"title": "双周实验", "weekday": 4, "periods": [5, 6], "location": "东教503",
...
def test_import_creates_events_with_conflict_report(db):
    ...
    assert len(events) == 2
    ev = next(e for e in events if e.title == "数值分析")
    assert ev.date == date(2026, 9, 15) and "UNTIL" in ev.recur_rrule and ev.location == "五教201"
...
def test_import_writes_human_readable_repeat_note(db):
    """re #020 事项2：每门课写入人类可读周次规则（repeat_note），
    ...
    assert events["数值分析"].repeat_note == "每周（第2-13周）"
...
def _slot(title, weekday, location="五教201"):
...
def test_same_title_location_different_weekday_both_created(db):
    """真实课表场景：数值分析5班 周二2-3节 与 周四2-3节 同教室 = 两条合法排课。
    旧判重键 (title, location) 把后者误跳过（reason=同名同地点已存在）；location 不参与判重。"""
    entries = [_slot("数值分析5班", 2), _slot("数值分析5班", 4)]
...
def test_db_existing_event_skips_only_matching_slot(db):
    ...
    assert [s["title"] for s in out["skipped"]] == ["数值分析5班"]
    assert all("已存在" in s["reason"] for s in out["skipped"])
    from zhishi.domain.models import Event
    assert db.query(Event).count() == 2
